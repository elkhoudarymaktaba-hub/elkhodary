...
export default function Sidebar() {
  ...
  return (
    <>
      ...
      <aside className="hidden lg:flex flex-col w-64 bg-[#0F172A] text-white fixed top-0 bottom-0 right-0 z-30 shadow-2xl border-l border-slate-800">
        
        {/* اللوجو والهوية */}
        <div className="p-6 border-b border-slate-800 flex flex-col items-center gap-3">
          <div className="w-16 h-16 bg-gradient-to-tr from-amber-600 to-amber-400 rounded-[16px] flex items-center justify-center text-3xl shadow-lg -rotate-3 hover:rotate-0 transition-transform duration-300">
            📖
          </div>
          <div className="text-center">
            <h1 className="font-black text-xl font-arabic tracking-wide text-amber-400 drop-shadow-sm">مكتبة الخضري</h1>
            <p className="text-xs text-slate-300 font-arabic mt-1 font-bold">لوحة التحكم الإدارية</p>
          </div>
        </div>
            ...
            return (
              <Link
                ...
                className={clsx(
                  ...
                  {
                    'bg-[#2E7FD9] text-white shadow-md font-bold border-r-4 border-amber-400': isActive,
                    'text-slate-300 hover:text-white hover:bg-slate-800/80': !isActive,
                  }
                )}
              >
        ...
        <div className="p-4 border-t border-slate-800">
          <button
            ...
            className="w-full flex items-center justify-center gap-2 px-4 py-3 bg-rose-500/10 hover:bg-rose-500/20 text-rose-300 hover:text-white rounded-[12px] text-sm font-semibold transition-all duration-200 font-arabic border border-rose-500/20"
          >
      ...
      <nav className="lg:hidden fixed bottom-0 left-0 right-0 bg-[#0F172A] text-white z-40 shadow-2xl border-t border-slate-800 flex overflow-x-auto scrollbar-none items-center justify-between px-2 py-1.5 h-16">
        {menuItems.map((item) => {
          const Icon = item.icon;
          const isActive = pathname === item.path;
          return (
            <Link
              ...
              className={clsx(
                ...
                {
                  'text-white bg-[#2E7FD9] border-t-2 border-amber-400 font-bold': isActive,
